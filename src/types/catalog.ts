...
export interface CatalogTier2 {
    ...
    character_archetypes: string[];                 // strong female lead, anti-hero, ensemble cast
    content_warnings: ContentWarning[];
    emotional_impact: CatalogEmotionalImpact;
    setting: CatalogSetting;
    target_age_group: CatalogAgeGroup;
    perfect_for?: string;                           // NEW
    memorable_quote?: string;                       // NEW
}
...
export interface BookCatalogEntry extends CatalogTier1, Partial<CatalogTier2>, Partial<CatalogTier3>, Partial<CatalogTier4> {
    ...
    // Embedding support
    embedding_text?: string;            // concatenated text for vector embedding
...
export function makeCatalogKey(title: string, author: string): string {
    return `${title.toLowerCase().trim()}|${(author || 'unknown').toLowerCase().trim()}`;
}
...
export function createMinimalCatalogEntry(data: Partial<BookCatalogEntry> & { title: string; author: string }): BookCatalogEntry {
    return {
        ...
        character_archetypes: data.character_archetypes || [],
        ...
        memorable_quote: data.memorable_quote,

        // Tier 3
        storyline_structure: data.storyline_structure || [],
        ...
        best_read_when: data.best_read_when || [],
        reading_difficulty: data.reading_difficulty,
        positive_content_notes: data.positive_content_notes || [],

        // Tier 4
        awards: data.awards || [],
        comparable_books: data.comparable_books || [],
        isbn: data.isbn || null,
        page_count: data.page_count || null,
        first_published_year: data.first_published_year || null,
