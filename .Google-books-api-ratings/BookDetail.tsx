...
export const BookDetailScreen = ({ route, navigation }: any) => {
    ...
    React.useEffect(() => {
        const fetchDetails = async () => {
            ...
            if (!needsDescription && !needsCover && !needsRating) return;

            // Step 1: Try Google Books
            if (needsDescription || needsCover || needsRating) {
                const details = await googleBooksService.searchBook(book.title, book.author);
                if (details) {
                    if (needsDescription && details.description) setDescription(details.description);
                    if (needsCover && details.coverImage) setCoverImage(details.coverImage);
                    if (needsRating && details.rating && details.rating > 0) {
                        setRating(details.rating);
                        setRatingsCount(details.ratings_count || 0);
                        setRatingSource(details.rating_source);
                    }
                }

                // Step 2: If still no rating after Google Books, try Open Library
                // Check current state via closure - need to use the details we just got
                const currentRating = (details?.rating && details.rating > 0) ? details.rating : rating;
                if (!currentRating || currentRating === 0) {
                    console.log(`[BookDetail] No Google rating for "${book.title}". Trying Open Library...`);
                    const olRating = await openLibraryService.fetchRating(book.title, book.author);
                    if (olRating && olRating.rating > 0) {
                        setRating(olRating.rating);
                        setRatingsCount(olRating.ratings_count);
                        setRatingSource(olRating.rating_source);
                    }
                }
            }
        };
            ...
            <View style={styles.headerContainer}>
                ...
                <View style={styles.headerContent}>
                    ...
                    <View style={styles.ratingContainer}>
                        ...
                        {ratingsCount > 0 && <Text style={styles.ratingCount}>({ratingsCount.toLocaleString()})</Text>}
